  ...
  private
    ...
    def section_props(section, cached_props:, request:, pundit_user:, seller_custom_domain_url:)
      ...
      when "SellerProfileProductsSection"
        ...
        cached_props[:search_results][:products] = Link.includes(ProductPresenter::ASSOCIATIONS_FOR_CARD_WITHOUT_USER).find(cached_props[:search_results][:products]).map do |product|
